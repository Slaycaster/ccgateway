<p align="center">
  <img src="assets/header.svg" alt="ccgateway — lean multi-agent gateway on top of Claude Code CLI" width="100%" />
</p>
...
Chat messages are **untrusted input** that become prompts the agent acts on. Two controls gate what happens next:

- **`allowedUsers`** (per gateway plugin) — authentication. Only listed users can message the agent.
- **`allowedTools`** (per agent) — authorization. The list is passed to `claude` as `--allowedTools`. Default for new agents is `Read, Grep, Glob` (read-only). Add `Bash`, `Write`, or `Edit` only if you trust every user in `allowedUsers` to run those on the host.
- **`dangerouslySkipPermissions`** (per agent) — defaults to `false` for new agents. Set to `true` only when `allowedTools` is tightly scoped. OpenClaw migrations and pre-0.3.1 agents default to `true` for back-compat — review them.

Full threat model and recommendations: [docs/concepts/architecture.md — Security model](docs/concepts/architecture.md#security-model).
